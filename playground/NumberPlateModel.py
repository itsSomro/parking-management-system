...
def convert_xml_to_yolo(xml_path, txt_path):
    ...
    # calculating image dim - for knowing how much of the image is the box
    size = root.find('size')
    ...
    with open(txt_path, 'w') as f:
        ...
        # this will store all images with image AND xml(converted to text) files only
        # we will ignore pairs which are missing either one
        valid_pairs=[]
        ...
        print(f"✅ Found {len(valid_pairs)} valid Image+XML pairs across all folders.")
        ...
        def process_data(pairs, subset_name):
            print(f"⚙️ Processing {subset_name} set ({len(pairs)} files)...")
