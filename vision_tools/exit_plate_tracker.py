...
# --------------------------------------------------------------------------
"""
 BACKGROUND THREAD: Keeps the camera feed running at real-time speed
 After the math for reading a frame, it reads the latest frame from the camera and throws away all the older queued up frames
"""
...
def main():
    ...
    cam_user = os.getenv("CAMERA_USER", "default_user")
    cam_pass = os.getenv("CAMERA_PASS", "default_pass")
    cam_base_url = os.getenv("CAMERA_URL", "http://127.0.0.1:8080/video")

    URL = cam_base_url.replace("http://", f"http://{cam_user}:{cam_pass}@")
    ...
    def license_complies_format(text):
        ...
        def validate_chunk(chunk):
            ...
            for i in range(0, 2):
                if clean_text[i] in number_to_letter:
                    clean_text[i] = number_to_letter[clean_text[i]]

            # 2. RTO Code (Numbers)
            for i in range(2, 4):
                if clean_text[i] in letter_to_number:
                    clean_text[i] = letter_to_number[clean_text[i]]
        ...
        # First, try to find a valid 10-character plate in the string
        for i in range(len(text) - 9):
            candidate = text[i:i + 10]
            valid_plate = validate_chunk(candidate)
            if valid_plate:
                return valid_plate

        # If no 10-character plate is found, try finding a 9-character plate
        for i in range(len(text) - 8):
            candidate = text[i:i + 9]
            valid_plate = validate_chunk(candidate)
            if valid_plate:
                return valid_plate
